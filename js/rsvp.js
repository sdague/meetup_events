...
(function() {
   ...
   , Assets = {
       ...
       , json2: "https://secure.meetup.com/script/1253729729552097/Meetup/api/json2.js"
   ...
   , C = (function() {
       ...
       , copy = {
         'en' : {
           ...
           , 'no-waitlist': 'There is no wait list'
           ...
           , 'rsvp-want-to-wait-btn': 'Put me on the waiting list'
   ...
   , T = {
     ...
     , css: function() {
         return ['.mu-widget :-moz-any-link:focus {outline:none;}'
              ...
              , '.mu-widget img, .mu-rsvp-btn img { width:auto; }'
              ...
              , '  -moz-border-radius: 3px;-webkit-border-radius: 3px;border-radius: 3px; padding:2px; display:block; }'
     ...
     , eventDetails: function(e, closed, content) {
       return T.box(
              ['<div class="dets">'
                 ...
                 , '<div class="mu-e-time">', T.date(e.time), '</div>'
                 , T.eventVenue(e)
                 , '</div>'
                 , T.feeInfo(e)
     ...
     , createProfile: function(u, g) {
       var jm = g.join_mode, i = g.join_info, qs = i.questions
         , qr = parseInt(i.questions_req, 10), ir = parseInt(i.intro_req, 10)
   ...
   mu.api = mu.api || (function(opts, muready) {
     ...
     , onToken = function(token, expires, state) {
       requireServer(function() {
         ...
         if(state) {
           ...
           var pstate = state.split('-').pop().split(',');
     ...
     var logout = function(token, callback) {
       ...
       , iframe = $('<iframe style="width:0px;height:0px;border:0px" scrolling="no" frameborder="no" allowtransparency="true" src="'+uri+'"></iframe>')
